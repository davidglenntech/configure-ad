...
- Link Client VM to a domain
- Setup Remote Desktop for non-administrative users on Client VM
- Create additional users and attempt to log in to Client VM as one of those newly created users
...
- Name your Virtual Machine any way you want (this example uses **DC-01**).
...
- Press the Windows key (or Start Button), type and select CMD or "Command Prompt" (you can run as Admin if desired).
<p align="center">
...
- Inside the Command Prompt, type "ping -t {DC-01 Private IP Address}" (this example uses IP address **10.0.0.4**)
  - _This will infinitely send data packets for response to the DC-01 VM._
- This confirms if the Client VM can see the Domain Controller VM successfully, otherwise you'll receive a "Request Timed Out" message.
...
- Back on the Server Manager, click on the flag icon with a caution symbol on it (located at the top-right header).
...
- Once completed, log back into the DC-01 VM.
  - _However, you should not be able to use the same username as before, now that it requires the domain name._
...
- Create a first and last name, as well as a login name for this Admin user, then click "Next" (this example uses **Jane Doe** / **jane_admin**)
...
_The user account is only inside a folder named `_ADMINS`, but that doesn't mean it has the privileges as one, so:_
...
- Type in the word "domain", then click "Check Names", allowing you to view all already built-in groups.
...
- Once completed, log off of Domain Controller VM and log on to the newly created admin account with the domain name (this example uses **mydomain.com\jane_admin**).
...
- Click on "Networking", then click on "Network Interface" (this example uses **client-01857**).
<p align="center">
...
- Click "OK".
- If done correctly, you should see a welcoming window appear to join the domain.
...
- When ready, at the top menu, click the "Run" button (green play button icon).
  - _This script will create 100 accounts using the password "Password1" (variables set at the beginning of code).
These accounts will be placed at their set path: `_EMPLOYEES`, listed near the end of the code._
<p align="center">
...
_Next, we're going to attempt to log in one of those random users to Client-01 VM._
- Copy any randomly created user (this example will use **did.cuta**).
- Logoff of any account currently on Client-01 VM, and attempt to log in with the random user.
